...
interface Props {
  ...
  | string
  // New prop for session management
  sessionValue?: {
    morning: boolean
    afternoon: boolean
    evening: boolean
...
const emit = defineEmits<{
  ...
  (e: 'update:sessionValue', value: { morning: boolean; afternoon: boolean; evening: boolean; night: boolean }): void;
...
const sessionData = reactive({
  morning: props.sessionValue?.morning || false,
  afternoon: props.sessionValue?.afternoon || false,
  evening: props.sessionValue?.evening || false,
  night: props.sessionValue?.night || false
})

// Watch for changes in sessionData and emit updates
watch(sessionData, (newValue) => {
  emit('update:sessionValue', { ...newValue })
}, { deep: true })

// Watch for external changes to sessionValue prop
watch(() => props.sessionValue, (newValue) => {
  if (newValue) {
    sessionData.morning = newValue.morning
    sessionData.afternoon = newValue.afternoon
    sessionData.evening = newValue.evening
...
function resetSessionData() {
  sessionData.morning = false
  sessionData.afternoon = false
  sessionData.evening = false
  sessionData.night = false
}

// Modify the setToToday function to also reset session
...
function applyDate(close?: (ref?: Ref | HTMLElement) => void) {
  emit('update:sessionValue', { ...sessionData })
...
function datepickerClasses(date: DatePickerDay) {
  ...
  if (active) {   
    classes = today
      ? 'text-vtd-dark-blue bg-vtd-medium-gray rounded-md'
      : disabled
        ? 'text-vtd-light-gray font-normal disabled:text-vtd-light-gray disabled:cursor-not-allowed'
        : (s && e) && (date.isBetween(s as Dayjs, e as Dayjs, 'date', '()'))
          ? 'text-vtd-dark-blue'
          : 'text-vtd-dark-blue'
  }
  if (off)
    classes = 'text-vtd-dark-blue disabled:cursor-not-allowed'

  if (s && e && !off) {
    if (date.isSame(s, 'date')) {
      classes = e.isAfter(s, 'date')
        ? 'bg-vtd-dark-blue text-white rounded-l-md disabled:cursor-not-allowed'
        : 'bg-vtd-dark-blue text-white rounded-r-md disabled:cursor-not-allowed'
      if (s.isSame(e, 'date')) {
        classes
          = 'bg-vtd-dark-blue text-white rounded-md disabled:cursor-not-allowed'
      }
    }
...
function betweenRangeClasses(date: Dayjs) {
  ...
  if (s && e) {
    if (date.isSame(s, 'date')) {
      if (e.isBefore(s))
        classes += ' rounded-r-md inset-0'

      if (s.isBefore(e))
        classes += ' rounded-l-md inset-0'
    }
...
function setToThisWeekend(close?: (ref?: Ref | HTMLElement) => void) {
  const today = dayjs();
  const day = today.day();  
  const daysToSaturday = (6 - day + 7) % 7;   
  const saturday = today.add(daysToSaturday, "day");
...
function setToThisWeek(close?: (ref?: Ref | HTMLElement) => void) {
  const today = dayjs();
  const day = today.day();  
  const sunday = today.subtract(day, "day");  
...
provide(setToThisWeekendKey, setToThisWeekend)
provide(setToThisWeekKey, setToThisWeek)
provide(setToTomorrowKey, setToTomorrow)
provide(setToThisMonthKey, setToThisMonth)
provide(setToThisYearKey, setToThisYear)
provide(setToCustomShortcutKey, setToCustomShortcut)

defineExpose({ clearPicker, resetSessionData })
